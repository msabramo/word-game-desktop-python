...
import datetime, gobject, pygtk, time, webbrowser
pygtk.require('2.0')
...
class WordUp(gtk.glade.XML):
    ...
    def update_time(self):
        if self.game and self.main_window.has_toplevel_focus():
    ...
    def on_new_activate(self, action=None):
        if True or sys.platform == 'win32': self.nag()
        
        if self.game.board.num_tiles <= 10:
            words_left = self.game.board.words_left(self.dictionary)
            if len(words_left) > 0:
